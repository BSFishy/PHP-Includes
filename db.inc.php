...
class db
{
	...
	function db($host, $dbname, $dbpass, $dbuser)
	{
		...
		$this->db = @mysql_connect($host, $dbuser, $dbpass, true);
	...
	function error()    {   if (mysql_errno($this->db)) return true; else return false;	}
	
	function insertid() {	return mysql_insert_id($this->db);	}
	
	function affected() {	return mysql_affected_rows($this->db);	}
